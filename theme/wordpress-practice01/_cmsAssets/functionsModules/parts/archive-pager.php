<?php
  ...
  if($currentTersm && $period){ // カテゴリ　＆　月間
    $pagerArgs["tax_query"] = $releases_args['tax_query'];
    $pagerArgs["date_query"] = $releases_args['data_query'];
    ...
    $query = new WP_Query( $pagerArgs );
    $page = ceil($query->found_posts / $gVars['post_per_page'][$posttype]);
  	// vd("B");
		// vd($query->found_posts);
    ...
    $post_count = wp_count_posts($posttype)->publish;
